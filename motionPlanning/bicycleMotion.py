...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(b)>0.001:
            ...
            result.orientation = (b+self.orientation)%(2*pi)
            result.x = cx+sin(b+self.orientation)*R
            result.y = cy-cos(b+self.orientation)*R

        else:
            result.x = self.x+d2*cos(self.orientation)
            result.y = self.y+d2*sin(self.orientation)
            result.orientation = (self.orientation+b)%(2*pi)
    ...
    def sense(self,addNoise = 1): #do not change the name of this function
        ...
        for i in [2,1,0,3]:
            bearing = (atan2(landmarks[i][1]-self.y,landmarks[i][0]-self.x)-self.orientation)%(2*pi)
            if addNoise:
